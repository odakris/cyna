...
export default function CategoryHomePage() {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      setLoading(true)
      const data: Category[] = await fetch("/api/categories").then(res =>
        res.json()
      )
      setCategories(data)
      setError(null)
  ...
  const table = useReactTable({
    ...
    filterFns: {}, // Add an empty filterFns object or define custom filter functions here
    state: {
      sorting,
      columnFilters,
      columnVisibility,
        ...
        <Card>
          <CardContent className="p-0">
            <Table>
              <TableHeader>
                <TableRow>
                  {Array(categoriesColumns.length)
                    .fill(0)
                    .map((_, i) => (
                      <TableHead key={i}>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            {categories.length} categorie{categories.length > 1 ? "s" : ""} dans
            la base de données
          </p>
        </div>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  ...
                  {Object.keys(rowSelection).length} catégorie
        ...
        <CardContent className="p-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0 py-4">
            <div className="text-sm text-muted-foreground">
              ...
              catégorie(s) sélectionné(s).
            </div>

            <div className="flex items-center space-x-6 lg:space-x-8">
              <div className="flex items-center space-x-2">
                <p className="text-sm font-medium">Catégories par page</p>
                <select
