...
export async function GET(request: NextRequest) {
  try {
    ...
    // Vérification d'autorisation simplifiée et corrigée
    const effectiveUserId = userId || guestId
    if (
      order.id_user !== effectiveUserId &&
      order.guest_id !== effectiveUserId
    ) {
      console.error("[Checkout Success API] Accès non autorisé:", {
        invoiceNumber,
        effectiveUserId,
      })
      return NextResponse.json(
    ...
    const response = await fetch(
      `${request.nextUrl.origin}${order.invoice_pdf_url}`,
      {
        method: "GET",
        headers: {
    ...
    // Correction du commentaire multiligne par un commentaire simple
