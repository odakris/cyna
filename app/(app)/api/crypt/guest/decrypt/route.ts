...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!userId && !(guestId && guestId !== "undefined")) {
      console.error(
        "[Decrypt API] Utilisateur non authentifié et aucun guestId fourni"
      )
    ...
    if (!addresses.length && !payments.length) {
      ...
      return NextResponse.json(
        { message: "Aucune donnée à déchiffrer" },
        { status: 400 }
      )
    }
    ...
    for (const address of addresses) {
      const order = await prisma.order.findFirst({
        where: {
          id_address: Number(address.id_address),
          ...(userId ? { id_user: Number(userId) } : { guest_id: guestId }),
        },
      })
      if (!order) {
    ...
    const decryptedAddresses = addresses.map(addr => {
      const decryptedAddr = {
        ...addr,
        first_name: addr.first_name.includes(":")
          ? decrypt(addr.first_name)
          : addr.first_name,
        last_name: addr.last_name.includes(":")
          ? decrypt(addr.last_name)
          : addr.last_name,
        address1: addr.address1.includes(":")
          ? decrypt(addr.address1)
          : addr.address1,
        address2:
          addr.address2 && addr.address2.includes(":")
            ? decrypt(addr.address2)
            : addr.address2,
        postal_code: addr.postal_code.includes(":")
          ? decrypt(addr.postal_code, true)
          : addr.postal_code,
        city: addr.city.includes(":") ? decrypt(addr.city) : addr.city,
        country: addr.country.includes(":")
          ? decrypt(addr.country)
          : addr.country,
        mobile_phone: addr.mobile_phone.includes(":")
          ? decrypt(addr.mobile_phone)
          : addr.mobile_phone,
      }

      if (
        !decryptedAddr.first_name ||
        !decryptedAddr.last_name ||
        !decryptedAddr.address1
      ) {
        console.error(
          "[Decrypt API] Données d'adresse incomplètes après déchiffrement:",
          { decryptedAddr }
        )
    ...
    const decryptedPayments = payments.map(pay => {
      const decryptedPay = {
        ...pay,
        card_name: pay.card_name.includes(":")
          ? decrypt(pay.card_name)
          : pay.card_name,
        last_card_digits: pay.last_card_digits.includes(":")
          ? decrypt(pay.last_card_digits)
          : pay.last_card_digits,
      }

      if (!decryptedPay.last_card_digits) {
        console.error(
          "[Decrypt API] Données de paiement incomplètes après déchiffrement:",
          { decryptedPay }
        )
        throw new Error(
          "Échec du déchiffrement: données de paiement incomplètes"
        )
      }
    ...
    console.log("[Decrypt API] Données déchiffrées:", {
      decryptedAddresses,
      decryptedPayments,
    })
