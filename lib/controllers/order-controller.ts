 ...
 */
export const getUserOrderHistoryForDisplay = async (id: string, req?: NextRequest): Promise<NextResponse> => {
    if (!id) {
        return NextResponse.json({ error: "User ID is required" }, { status: 400 });
    }
    ...
    try {
        ...
        if (isNaN(userId) || userId <= 0) {
            return NextResponse.json({ error: "User ID must be a valid positive number" }, { status: 400 });
        }

        // Récupérer les filtres des paramètres de requête si req est fourni
        let filters;
        if (req) {
