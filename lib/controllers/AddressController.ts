...
export class AddressController {
  ...
  static async getUserAddressById(id: string, id_address: string) {
    if (!id || !id_address) {
      return NextResponse.json(
        { error: "User ID et Address ID requis" },
        { status: 400 }
      )
    }
    ...
    try {
      ...
      console.error(
        "Erreur lors de la récupération de l'adresse spécifique :",
        error
      )
      return NextResponse.json(
        { error: "Erreur serveur lors de la récupération de l'adresse" },
        { status: 500 }
      )
    }
  }
  ...
  static async createAddress(userId: string, data: any) {
    if (!userId || !data) {
      return NextResponse.json(
        { error: "User ID et données requises" },
        { status: 400 }
      )
    }
    ...
    try {
      ...
      console.error(
        "Erreur lors de la création de l'adresse (controller):",
        error
      )
      return NextResponse.json(
  ...
  static async updateAddress(id: string, id_address: string, data: any) {
    if (!id || !id_address || !data) {
      return NextResponse.json(
        { error: "User ID, Address ID, and data are required" },
        { status: 400 }
      )
    }
