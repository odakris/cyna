  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          message:
            "Non autorisé. Vous ne pouvez pas accéder aux données d'un autre utilisateur.",
        },
        { status: 403 }
      )
    }
...
export const paymentController = {
  async getPayment(
    req: NextRequest,
    { params }: { params: Promise<{ id: string; id_payment: string }> }
  ) {
    ...
    return withAuth(req, userId, async (userId: number) => {
      try {
        console.log(
          "[PaymentController getPayment] Récupération du paiement pour userId:",
          userId,
          "paymentId:",
          paymentId
        )
        ...
        if (!payment) {
          console.log("[PaymentController getPayment] Paiement non trouvé:", {
            userId,
            paymentId,
          })
          return NextResponse.json(
        ...
        console.log("[PaymentController getPayment] Paiement déchiffré:", {
          userId,
          paymentId,
        })
    ...
    return withAuth(req, userId, async (userId: number) => {
      try {
        ...
        if (payments.length === 0) {
          console.log(
            "[PaymentController getPayments] Aucun paiement trouvé pour userId:",
            userId
          )
  ...
  async createPaymentMethod(
    ...
    return withAuth(req, userId, async (userId: number) => {
      try {
        ...
        console.log(
          "[PaymentController createPaymentMethod] Données reçues:",
          body
        )
        ...
        if (last_card_digits.length !== 4) {
          ...
          return NextResponse.json(
            {
              message:
                "Les derniers chiffres de la carte doivent être exactement 4",
            },
            { status: 400 }
          )
        }
        ...
        if (stripe_customer_id) {
          if (
            ...
            return NextResponse.json(
              {
                message:
                  "L'identifiant Stripe fourni ne correspond pas à celui de l'utilisateur",
              },
              { status: 400 }
            )
          }
        ...
        if (!stripeCustomerId) {
          ...
          console.log(
            "[PaymentController createPaymentMethod] Client Stripe créé:",
            {
              userId,
        ...
        try {
          ...
          console.log(
            "[PaymentController createPaymentMethod] Payment method attaché:",
            {
              stripe_payment_id,
              stripeCustomerId: stripeCustomerId,
            }
          ...
          return NextResponse.json(
            {
              message:
                "Erreur lors de l'association du moyen de paiement au client Stripe",
        ...
        if (is_default) {
          ...
          console.log(
            "[PaymentController createPaymentMethod] Autres méthodes par défaut désactivées pour userId:",
            userId
          )
        }
        ...
        console.log(
          "[PaymentController createPaymentMethod] Données chiffrées:",
          paymentData
        )
        ...
        console.log(
          "[PaymentController createPaymentMethod] Moyen de paiement créé:",
          {
        ...
        console.log(
          "[PaymentController createPaymentMethod] Réponse envoyée:",
          decryptedPayment
        )
  ...
  async updatePaymentMethod(
    ...
    return withAuth(req, userId, async (userId: number) => {
      try {
        const body = await req.json()
        console.log(
          "[PaymentController updatePaymentMethod] Données reçues:",
          body
        )
        ...
        try {
          ...
          console.log(
            "[PaymentController updatePaymentMethod] Payment method attaché:",
            {
              stripe_payment_id,
        ...
        console.log(
          "[PaymentController updatePaymentMethod] Données chiffrées:",
          paymentData
        )

        const updatedPayment = await paymentService.replacePayment(
          userId,
          paymentId,
          paymentData
        )
        console.log(
          "[PaymentController updatePaymentMethod] Paiement mis à jour:",
          updatedPayment
        )
        ...
        console.log(
          "[PaymentController updatePaymentMethod] Réponse envoyée:",
          decryptedPayment
        )
        return NextResponse.json(decryptedPayment, { status: 200 })
      } catch (error: any) {
  ...
  async deletePaymentMethod(
    ...
    return withAuth(req, userId, async (userId: number) => {
      try {
        console.log(
          "[PaymentController deletePaymentMethod] Suppression du paiement pour userId:",
          userId,
          "paymentId:",
          paymentId
        )
        ...
        console.log(
          "[PaymentController deletePaymentMethod] Paiement supprimé:",
          { userId, paymentId }
        )
        return NextResponse.json(
