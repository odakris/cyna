...
export const getUserByEmail = async (email: string): Promise<User | null> => {
    try {
        ...
        console.log("getUserByEmail - User found:", { id_user: user.id_user, email: user.email })
        return user
    } catch (error) {
        console.error("Erreur lors de la récupération de l'utilisateur par email:", error)
        throw error instanceof Error ? error : new Error(`Erreur lors de la récupération de l'utilisateur par email`)
    }
}
